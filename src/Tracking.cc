...
Tracking::Tracking(
    ...
    const int sensor):                  // 传感器类型
        mState(NO_IMAGES_YET),                              // 当前系统状态：NO_IMAGES_YET
        mSensor(sensor),                                    // 传感器类型：sensor
        mbOnlyTracking(false),                              // 纯追踪模式：false
        mbVO(false),                                        // 当处于纯追踪模式的时候，这个变量表示了当前跟踪状态的好坏
        mpORBVocabulary(pVoc),                              // BoW字典
        mpKeyFrameDB(pKFDB),                                // 关键帧产生的词袋数据库
        mpInitializer(static_cast<Initializer*>(NULL)),     // 暂时给地图初始化器设置为空指针NULL
        mpSystem(pSys),                                     // 系统实例
        mpViewer(NULL),                                     // 注意可视化的查看器是可选的，因为ORB-SLAM2最后是被编译成为一个库
        mpFrameDrawer(pFrameDrawer),                        // 帧绘制器
        mpMapDrawer(pMapDrawer),                            // 地图点绘制器
        mpMap(pMap),                                        // 地图句柄
        mnLastRelocFrameId(0)                               // 恢复为0，没有执行这个过程时的默认值
{
...
/**
 * 输入：左目图像RGB或者RGBA图像
 * 1.将图像转为mImGray并初始化mCurrentFram
 * 2.进行Tracking过程
